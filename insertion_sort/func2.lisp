    ...
    (LOOP for i from 0 to old_leng DO
        ...
        (LOOP for j from 0 to ( - (LENGTH new_list) 1) DO
            ...
            (COND
                ...
                ; 정렬된 리스트에 key보다 큰 원소가 존재하는 경우
                ((< key atm)

                    (FORMAT t "~D th atom ~D is bigger than key ~D ~%" (+ j 1) atm key)

                    ; 현재 인덱스 j를 기준으로, 첫번째 원소부터 j-1 번째 원소를 포함하는 서브리스트 head와 
                    ; j부터 마지막 원소를 포함하는 서브리스트 tail 을 생성.
                    ; head 와 key 와 tail을 하나의 리스트로 합쳐 new_list에 할당.
        ...
        ; 안쪽 loop 에서 삽입이 이루어지지 않았다는 것은 key가 정렬된 리스트의 모든 요소들 보다 크다는 것을 의미.
        (IF (EQ ni_flag T)
            (PROGN
